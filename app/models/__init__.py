...
class ModuleRegistration(Base):
    ...
    event_id: Mapped[Optional[str]] = mapped_column(String(64), unique=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
...
class ModuleEventOutbox(Base):
    __tablename__ = "module_event_outbox"
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    ...
    __table_args__ = (
        UniqueConstraint("company_id", "idempotency_key", name="uq_module_event_idempotency"),
    )
