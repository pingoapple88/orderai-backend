...
- [ ] **Expo P0 scenario completeness 核對**：確認既有 parse／risk／queue `DEMO_MOCK` handoff 是否已可用單一跨screen索引提供高信心 success、低於0.85 threshold 的 `needs_review`、unknown／`manual_review`，且每一情境都可追溯 route、fixture、output、human action、fallback、source／rollback及evidence；只有缺口存在才新增 canonical revision。
